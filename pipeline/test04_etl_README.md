...
- **Target System**: `destination_system` (bigquery)
  - Destination Table: `analytics.test04`
...
The container boots, runs `python -m pipeline.run_test04_etl`, loads data into the
...
## Running Locally
```bash
python -m pipeline.run_test04_etl --date $(date +%Y-%m-%d)
```

## Re-running the Pipeline
```bash
gcloud run jobs execute test04_etl --region us-central1
```
