# Pipeline: postgres_to_bigquery_kttest
...
The container boots, runs `python -m pipeline.run_postgres_to_bigquery_kttest`, loads data into the
...
## Running Locally
```bash
python -m pipeline.run_postgres_to_bigquery_kttest --date $(date +%Y-%m-%d)
```

## Re-running the Pipeline
```bash
gcloud run jobs execute postgres_to_bigquery_kttest --region us-central1
```
