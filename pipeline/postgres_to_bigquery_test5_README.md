# Pipeline: postgres_to_bigquery_test5
...
- **Target System**: `destination_system` (bigquery)
  - Destination Table: `analytics.test5`
...
The container boots, runs `python -m pipeline.run_postgres_to_bigquery_test5`, loads data into the
...
## Running Locally
```bash
python -m pipeline.run_postgres_to_bigquery_test5 --date $(date +%Y-%m-%d)
```

## Re-running the Pipeline
```bash
gcloud run jobs execute postgres_to_bigquery_test5 --region us-central1
```
