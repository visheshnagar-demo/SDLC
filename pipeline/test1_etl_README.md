...
The container boots, runs `python -m pipeline.run_test1_etl`, loads data into the
...
## Running Locally
```bash
python -m pipeline.run_test1_etl --date $(date +%Y-%m-%d)
```

## Re-running the Pipeline
```bash
gcloud run jobs execute test1_etl --region us-central1
```
