# Pipeline: cloudsql_postgres_to_bigquery_etl
...
- **Target System**: `destination_system` (bigquery)
  - Destination Table: `analytics.postgres_test1`
...
The container boots, runs `python -m pipeline.run_cloudsql_postgres_to_bigquery_etl`, loads data into the
...
## Running Locally
```bash
python -m pipeline.run_cloudsql_postgres_to_bigquery_etl --date $(date +%Y-%m-%d)
```

## Re-running the Pipeline
```bash
gcloud run jobs execute cloudsql_postgres_to_bigquery_etl --region us-central1
```
