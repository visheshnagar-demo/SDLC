...
- **Target System**: `destination_system` (bigquery)
  - Destination Table: `analytics.test3`
...
The container boots, runs `python -m pipeline.run_test3_etl`, loads data into the
...
## Running Locally
```bash
python -m pipeline.run_test3_etl --date $(date +%Y-%m-%d)
```

## Re-running the Pipeline
```bash
gcloud run jobs execute test3_etl --region us-central1
```
