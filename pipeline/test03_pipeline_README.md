...
- **Target System**: `destination_system` (bigquery)
  - Destination Table: `analytics.test03`
...
The container boots, runs `python -m pipeline.run_test03_pipeline`, loads data into the
...
## Running Locally
```bash
python -m pipeline.run_test03_pipeline --date $(date +%Y-%m-%d)
```

## Re-running the Pipeline
```bash
gcloud run jobs execute test03_pipeline --region us-central1
```
