# Pipeline: gcs_sales_to_bigquery
...
- **Target System**: `destination_system` (bigquery)
  - Destination Table: `analytics.harshada_test4`
  - Write Mode: `append`
  - Merge Keys: `[]`
...
The container boots, runs `python -m pipeline.run_gcs_sales_to_bigquery`, loads data into the
...
## Running Locally
```bash
python -m pipeline.run_gcs_sales_to_bigquery --date $(date +%Y-%m-%d)
```

## Re-running the Pipeline
```bash
gcloud run jobs execute gcs_sales_to_bigquery --region us-central1
```
