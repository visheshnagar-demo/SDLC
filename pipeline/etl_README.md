...
- **Target System**: `destination_system` (bigquery)
  - Destination Table: `analytics.test02`
...
The container boots, runs `python -m pipeline.run_etl`, loads data into the
...
## Running Locally
```bash
python -m pipeline.run_etl --date $(date +%Y-%m-%d)
```

## Re-running the Pipeline
```bash
gcloud run jobs execute etl --region us-central1
```
