# Pipeline: gcs_to_bigquery_test2
...
- **Target System**: `destination_system` (bigquery)
  - Destination Table: `analytics.test2`
...
The container boots, runs `python -m pipeline.run_gcs_to_bigquery_test2`, loads data into the
...
## Running Locally
```bash
python -m pipeline.run_gcs_to_bigquery_test2 --date $(date +%Y-%m-%d)
```

## Re-running the Pipeline
```bash
gcloud run jobs execute gcs_to_bigquery_test2 --region us-central1
```
