...
- **Target System**: `destination_system` (bigquery)
  - Destination Table: `analytics.test1`
...
The container boots, runs `python -m pipeline.run_gcs_to_bq_test1`, loads data into the
...
## Running Locally
```bash
python -m pipeline.run_gcs_to_bq_test1 --date $(date +%Y-%m-%d)
```

## Re-running the Pipeline
```bash
gcloud run jobs execute gcs_to_bq_test1 --region us-central1
```
