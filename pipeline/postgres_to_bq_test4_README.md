...
- **Target System**: `destination_system` (bigquery)
  - Destination Table: `analytics.test4`
...
The container boots, runs `python -m pipeline.run_postgres_to_bq_test4`, loads data into the
...
## Running Locally
```bash
python -m pipeline.run_postgres_to_bq_test4 --date $(date +%Y-%m-%d)
```

## Re-running the Pipeline
```bash
gcloud run jobs execute postgres_to_bq_test4 --region us-central1
```
