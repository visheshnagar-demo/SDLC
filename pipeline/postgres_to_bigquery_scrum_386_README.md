# Pipeline: postgres_to_bigquery_scrum_386
...
- **Target System**: `destination_system` (bigquery)
  - Destination Table: `analytics.postgres_test4`
  - Write Mode: `append`
  - Merge Keys: `[]`
...
The container boots, runs `python -m pipeline.run_postgres_to_bigquery_scrum_386`, loads data into the
...
## Running Locally
```bash
python -m pipeline.run_postgres_to_bigquery_scrum_386 --date $(date +%Y-%m-%d)
```

## Re-running the Pipeline
```bash
gcloud run jobs execute postgres_to_bigquery_scrum_386 --region us-central1
```
