...
- **Target System**: `destination_system` (bigquery)
  - Destination Table: `analytics.harshada-test2`
  - Write Mode: `append`
  - Merge Keys: `[]`
...
The container boots, runs `python -m pipeline.run_sales_etl`, loads data into the
...
## Running Locally
```bash
python -m pipeline.run_sales_etl --date $(date +%Y-%m-%d)
```

## Re-running the Pipeline
```bash
gcloud run jobs execute sales_etl --region us-central1
```
