"""Standalone Connector Pipeline: postgres_test_data_to_bigquery
...
logger = logging.getLogger("postgres_test_data_to_bigquery")
...
class PipelineRunner:
    def __init__(self, execution_date: str = None):
        ...
        self.staging_dir = os.path.join("staging", "postgres_test_data_to_bigquery", self.execution_date)
    ...
    def extract(self) -> int:
        ...
        source_tbl = os.getenv("SOURCE_TABLE", "test_data")
        ...
        try:
            ...
            logger.error("Failed executing query on database: %s", query_err)
            raise RuntimeError(f"Query execution failed: {query_err}") from query_err
        finally:
    ...
    def transform(self) -> int:
        ...
        for col in df.select_dtypes(include=["object", "string"]).columns:
            df[col] = df[col].astype(str).str.strip().replace({"nan": None, "None": None, "<NA>": None, "NULL": None, "": None})
        ...
        if raw_count > 0 and len(df_valid) == 0:
            logger.error(
                ...
                "This indicates a schema contract violation. Failing pipeline.", raw_count
            )
            sys.exit(1)

        df_valid = df_valid.copy()
        df_valid["_etl_loaded_at"] = datetime.now(timezone.utc)
        df_valid["_etl_source_instance"] = os.getenv("INSTANCE_CONNECTION_NAME", "sdlc-etl-demo-db").split(":")[-1]
    ...
    def load(self) -> bool:
        ...
        try:
            ...
            logger.error("Dataset check failed for '%s': %s", dataset_id, ds_err)
            raise RuntimeError(f"BigQuery dataset creation failed: {ds_err}") from ds_err
        ...
        if schema_file:
            try:
                ...
                logger.info("Attached reconciled BigQuery schema (%d fields)", len(reconciled_schema))
            except Exception as schema_err:
                logger.error("Failed loading BigQuery schema from %s: %s", schema_file, schema_err)
                raise RuntimeError(f"BigQuery schema load failed: {schema_err}") from schema_err
        else:
        ...
        try:
            ...
            logger.error("BigQuery load failed: %s", load_err)
    ...
    def run(self) -> int:
        ...
        try:
            ...
            if records > 0 and valid_count == 0:
                logger.error("Extracted %d records but 0 survived transformation. Failing pipeline.", records)
                return 1
            self.load()
            ...
            raise RuntimeError(f"Pipeline run error: {exc}") from exc
...
if __name__ == "__main__":
    ...
    try:
        ...
        logging.getLogger("postgres_test_data_to_bigquery").critical("Pipeline failed: %s", exc, exc_info=True)
        sys.exit(1)
