...
class PipelineRunner:
    ...
    def transform(self) -> int:
        ...
        # Deduplication on order_id if present
        if "order_id" in df.columns:
            df = df.drop_duplicates(subset=["order_id"], keep="last")

        # Derive order_date partition field if created_at is present
    ...
    def load(self) -> bool:
        ...
        """
        logger.info("Loading staged file %s into bigquery (analytics.harshada-test1)...", self.staging_file)
        if not os.path.exists(self.staging_file) or os.path.getsize(self.staging_file) == 0:
            logger.warning("Staging file %s is empty or missing. Skipping load phase.", self.staging_file)
            return True
        project_id = os.getenv("GCP_PROJECT") or os.getenv("GCP_PROJECT_ID") or os.getenv("PROJECT_ID") or os.getenv("GOOGLE_CLOUD_PROJECT") or "upbeat-repeater-477110-q6"
        dataset_id = os.getenv("BIGQUERY_DATASET") or "analytics"
        table_id = os.getenv("BIGQUERY_TABLE") or "harshada-test1"
        write_mode = "append"

        if not project_id:
            raise EnvironmentError("FATAL: GCP_PROJECT or PROJECT_ID must be set for BigQuery loading.")
        ...
        schema_file = None
        for cand in [
            os.path.join("schemas", "harshada-test1_schema.json"),
        ...
        if schema_file:
            try:
                ...
                logger.error("Could not attach schema from %s: %s", schema_file, schema_err)
                raise RuntimeError(f"Failed to attach schema from {schema_file}: {schema_err}") from schema_err
