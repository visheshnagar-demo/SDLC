...
_Auto-generated by the SDLC Assistant from the generated codebase and the approved Feature Spec (latest: SCRUM-364). Regenerated on each push._

## System Diagram
```mermaid
graph TD
  User([User])
  UI["N/A (Backend Batch Data Pipeline)<br/>client/"]
  API["FastAPI Backend<br/>server/"]
  DB[("Cloud SQL PostgreSQL / Google BigQuery")]
  User --> UI
  UI -->|HTTP / JSON| API
  API --> DB
  DB --- tbl_test_data["test_data"]
  DB --- tbl_analytics_postgres_test2["analytics.postgres_test2"]
```
...
- **orm**: SQLAlchemy 2.x
- **database**: Cloud SQL PostgreSQL / Google BigQuery
- **frontend**: N/A (Backend Batch Data Pipeline)
...
- GET /api/v1/etl/jobs/status

## Data Model
- test_data
- analytics.postgres_test2
