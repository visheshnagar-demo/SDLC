"""Auto-Boot Cloud Run Server for gcs_csv_to_bigquery_etl ETL Pipeline.
...
logger = logging.getLogger("gcs_csv_to_bigquery_etl_server")

PORT = int(os.environ.get("PORT", 8080))
EXECUTION_STATUS = {
    "service": "gcs_csv_to_bigquery_etl",
    "status": "INITIALIZING",
    "records_processed": 0,
    "last_run": None,
    "error": None,
...
def _run_pipeline_task():
    ...
    logger.info("Auto-Boot: Triggering one-time ETL execution for gcs_csv_to_bigquery_etl...")
    try:
...
class PipelineStatusHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path in ("/", "/status", "/healthz", "/api/v1/etl/health"):
    ...
    def do_POST(self):
        if self.path in ("/run", "/api/v1/etl/jobs/run"):
            threading.Thread(target=_run_pipeline_task, daemon=True).start()
            self.send_response(202)
            self.send_header("Content-Type", "application/json")
            self.end_headers()
            self.wfile.write(json.dumps({"message": "ETL pipeline execution triggered", "status": "QUEUED"}, indent=2).encode("utf-8"))
        else:
            self.send_response(404)
            self.end_headers()
...
def main():
    ...
    logger.info("Serving gcs_csv_to_bigquery_etl auto-boot HTTP server on port %d...", PORT)
