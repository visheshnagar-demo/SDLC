"""Auto-Boot Cloud Run Server for gcs_to_bigquery_analytics ETL Pipeline.
...
logger = logging.getLogger("gcs_to_bigquery_analytics_server")

PORT = int(os.environ.get("PORT", 8080))
EXECUTION_STATUS = {
    "service": "gcs_to_bigquery_analytics",
    "status": "INITIALIZING",
    "records_processed": 0,
    "last_run": None,
    "error": None,
...
def _run_pipeline_task():
    ...
    logger.info("Auto-Boot: Triggering one-time ETL execution for gcs_to_bigquery_analytics...")
    try:
        from pipeline.run_gcs_to_bigquery_analytics import PipelineRunner
...
def main():
    ...
    logger.info("Serving gcs_to_bigquery_analytics auto-boot HTTP server on port %d...", PORT)
