...
def test_create_cloud_sql_engine(mock_env):
    ...
    with patch("pipeline.extractor.Connector", mock_connector_cls), \
         patch("pipeline.extractor.sqlalchemy", mock_sqlalchemy):
        config = PipelineConfig.from_env()
        create_cloud_sql_engine(config)
        assert mock_connector_cls.called
        assert mock_sqlalchemy.create_engine.called
...
def test_loader_dataset_and_table_creation(mock_env):
    ...
    mock_bq_client.get_dataset.side_effect = FakeNotFound("Dataset not found")
    mock_bq_client.get_table.side_effect = [FakeNotFound("Table not found"), MagicMock(num_rows=2)]
...
def test_run_pipeline_end_to_end(mock_env):
    ...
    with patch("pipeline.extractor.create_cloud_sql_engine"), \
