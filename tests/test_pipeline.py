"""Automated tests for Sales ETL pipeline modules and scripts."""
import ast
import os
import pytest
from datetime import datetime, date

from pipeline.run_sales_etl import (
    SalesDataCleanser,
    QuarantineManager,
    PostgresExtractor,
    BigQueryLoader,
...
import server.pipeline.cleanser as server_cleanser
import server.pipeline.quarantine as server_quarantine
import server.pipeline.extractor as server_extractor
import server.pipeline.loader as server_loader
import server.pipeline.main as server_main
...
def test_schema_json_exists():
    """Verifies that target schema definitions exist and are valid JSON."""
    import json
    schema_paths = [
        os.path.join("schemas", "sales_order_schema.json"),
        os.path.join("schemas", "fct_sales_orders_v1_schema.json"),
    ]
...
def test_email_validation_logic():
    """Tests RFC-compliant email validation rules in both pipeline and server packages."""
    for cleanser_cls in [SalesDataCleanser, server_cleanser.SalesDataCleanser]:
        assert cleanser_cls.is_valid_email("alice@example.com") is True
        assert cleanser_cls.is_valid_email("user.name+tag@sub.domain.co.uk") is True
        assert cleanser_cls.is_valid_email("john.doe@company.org") is True

        assert cleanser_cls.is_valid_email("") is False
        assert cleanser_cls.is_valid_email(None) is False
        assert cleanser_cls.is_valid_email("invalid-email") is False
        assert cleanser_cls.is_valid_email("user@") is False
        assert cleanser_cls.is_valid_email("@domain.com") is False
        assert cleanser_cls.is_valid_email("user@domain") is False
        assert cleanser_cls.is_valid_email("user @domain.com") is False


def test_amount_validation_logic():
    """Tests monetary amount validation rules (must be non-null and > 0)."""
    for cleanser_cls in [SalesDataCleanser, server_cleanser.SalesDataCleanser]:
        assert cleanser_cls.is_valid_amount(100.50) is True
        assert cleanser_cls.is_valid_amount("250.00") is True
        assert cleanser_cls.is_valid_amount(0.01) is True

        assert cleanser_cls.is_valid_amount(None) is False
        assert cleanser_cls.is_valid_amount(0) is False
        assert cleanser_cls.is_valid_amount(-15.50) is False
        assert cleanser_cls.is_valid_amount("invalid_amount") is False
        assert cleanser_cls.is_valid_amount(float("nan")) is False
