...
def run_pipeline_job(request_params: PipelineRunRequest, db: Session = None) -> PipelineRunResponse:
    ...
    try:
        ...
        valid_records, metrics_dict, quarantined_records = DataValidator.validate_batch(raw_records)

        transformed_records = DataTransformer.transform_batch(valid_records)

        loader = BigQueryLoader()
        loaded_count = loader.load_records(transformed_records, force_reload=request_params.force_reload)
        ...
        log_audit_event("PIPELINE_RUN_COMPLETED", {
            "job_id": job_id,
...
async def lifespan(app: FastAPI):
    ...
    if settings.AUTO_BOOT_ETL:
        def auto_boot_worker():
            pipeline_logger.info("Auto-Boot ETL hook triggered. Running initial sync...")
            try:
